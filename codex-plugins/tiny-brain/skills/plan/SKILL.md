...
**2. Dispatch both reviewers** with the Codex agent delegation (report-only — neither edits the PRD):

- `Codex role: matching specialist reviewer`:
  ```
  Review the deliverability of:
  - PRD: <prd-slug>
  ```
- `Codex role: architecture-reviewer` (the PRD template always carries a
...
`matching specialist reviewer` agent enforces the same rubric at design altitude — authoring to
