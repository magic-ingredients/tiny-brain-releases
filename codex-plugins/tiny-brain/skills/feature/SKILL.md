...
`matching specialist reviewer` agent — this is the enforcement pass for the Deliverability
section below, and it runs every time.

Use the Codex agent delegation with `Codex role: matching specialist reviewer`, naming the PRD and
...
`matching specialist reviewer` agent enforces the same rubric at design altitude.
