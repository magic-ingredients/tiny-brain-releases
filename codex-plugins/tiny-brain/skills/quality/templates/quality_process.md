...
Main Conversation (thin orchestrator, ~10-15K tokens)
  ...
  |-- Layer 2: 4 specialist delegated Codex agents (background) -> write domain.json files
...
| Agent (Codex role) | Model | Categories | Checklists | File Types |
|---|---|---|---|---|
| `a matching Tiny Brain review role` | opus | Security | SEC-* (7) | Source files |
| `a matching Tiny Brain review role` | sonnet | Performance, Reliability | PERF-* (5), REL-* (6) | Source files |
| `a matching Tiny Brain review role` | sonnet | Testing | TEST-* (4) | Test + source files |
| `a matching Tiny Brain review role` | sonnet | Maint, Arch, Doc, Ops | MAINT-* (6), ARCH-* (5), DOC-* (4), OPS-* (4) | Source files |
...
2. **4 specialist delegated Codex agents** with `run in parallel when the Codex agent surface supports it` - each reads `files.txt` from the run directory

delegation prompts are minimal (~10 lines each). Specialist agents have their domain checklists built into their agent definitions - no need to embed checklists in the prompt.
...
2. Use **the delegated agent final response** with each agent's task_id to check completion - agents' final messages include summary counts, so no need to Read full JSON files
...
Each specialist agent completion is checked via **the delegated agent final response**. The agent's final message includes summary counts. No JSON files are read into the main context.
