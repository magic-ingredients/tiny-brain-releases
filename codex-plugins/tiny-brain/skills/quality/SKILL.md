...
Main Conversation (thin orchestrator)
  ...
  |-- Agent steps (no step.analyzer): LLM delegated Codex agents (background)
...
- **Agent steps** (no `step.analyzer`): Spawn as LLM delegated Codex agents.
...
2. Run `tiny-brain config preferences get qualityPipeline` to get the configured quality pipeline steps. The output is a JSON array of step objects:
   ```json
   [
     {"type":"coverage","agent":"a matching Tiny Brain review role","analyzer":"coverage",...},
     {"type":"security","agent":"a matching Tiny Brain review role",...},
     ...
   ]
   ```
   Parse this array and partition into two lists:
   - **Analyzer steps**: entries where `analyzer` field is present (these have `"agent":"a matching Tiny Brain review role"` — ignore the agent field, use the CLI instead)
   - **Agent steps**: entries where `analyzer` field is NOT present (use the `agent` field to spawn the delegated Codex agent, use the `type` field as the step type for persist)
...
Launch ALL operations in a **single message** — analyzers via CLI, agents via Codex agent delegation.
...
**Agent steps** — launch each as a background delegated Codex agent (all in the same message, parallel):
...
Codex agent delegation:
  Codex role: "{step.agent}"
  run in parallel when the Codex agent surface supports it
  prompt: |
...
2. For each specialist agent: use `the delegated agent final response` with the agent's task_id to check completion
...
### View History
```
the Tiny Brain MCP quality tool({
  operation: "history",
  limit: 10
})
```
...
```
the Tiny Brain MCP quality tool({ operation: "implement-plan", planId: "<planId>" })
```
...
MCP equivalent:
```
the Tiny Brain MCP quality tool({ operation: "implement-plan", planId: "2026-02-09T14-30-plan" })
```
