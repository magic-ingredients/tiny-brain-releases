...
`tb work add spike <slug> "<title>"` CREATES docs/spikes/<slug>.md with the
...
Tasks are created with `tb work add task --spike <slug> "<description>"`. The CLI
...
<!--
  Tasks are created with `tb work add task --spike <slug> "<description>"`,
  ...
  - DO NOT hand-author the `### N.` / `id:` lines — `tb work add task` writes
