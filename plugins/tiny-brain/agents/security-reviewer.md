...
```bash
tiny-brain _review persist security --quality --json '<your-json>'
```
...
```bash
tiny-brain _review persist security --sha <SHA> --fix "<fix>" --task-id "<task>" --advance --json '<your-json>'
```

**If the commit has `PRD:` and `Feature:` headers:**

```bash
tiny-brain _review persist security --sha <SHA> --prd "<prd>" --feature "<feature>" --task-id "<task>" --advance --json '<your-json>'
```
