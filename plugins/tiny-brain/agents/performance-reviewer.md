...
```bash
tiny-brain _review persist performance --quality --json '<your-json>'
```
...
```bash
tiny-brain _review persist performance --sha <SHA> --fix "<fix>" --task-id "<task>" --advance --json '<your-json>'
```

**If the commit has `PRD:` and `Feature:` headers:**

```bash
tiny-brain _review persist performance --sha <SHA> --prd "<prd>" --feature "<feature>" --task-id "<task>" --advance --json '<your-json>'
```
