...
```bash
tiny-brain _review persist tdd-compliance --quality --json '<your-json>'
```
...
```bash
tiny-brain _review persist tdd-compliance --sha <SHA> --fix "<fix>" --task-id "<task>" --advance --json '<your-json>'
```

**If the commit has `PRD:` and `Feature:` headers:**

```bash
tiny-brain _review persist tdd-compliance --sha <SHA> --prd "<prd>" --feature "<feature>" --task-id "<task>" --advance --json '<your-json>'
```
