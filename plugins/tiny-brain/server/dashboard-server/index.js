...
var extractGroupsFromPath = (path11) => {
  const groups = [];
  path11 = path11.replace(/\{[^}]+\}/g, (match3, index) => {
    const mark = `@${index}`;
...
var getPath = (request) => {
  ...
  for (; i < url.length; i++) {
    ...
    if (charCode === 37) {
      ...
      const path11 = url.slice(start, queryIndex === -1 ? void 0 : queryIndex);
...
var Hono = class _Hono {
  ...
  */
  ...
  #addRoute(method, path11, handler) {
    method = method.toUpperCase();
    path11 = mergePath(this._basePath, path11);
    const r = { basePath: this._basePath, path: path11, method, handler };
...
var Trie = class {
  #context = { varIndex: 0 };
  #root = new Node();
  insert(path11, index, pathErrorCheckOnly) {
    const paramAssoc = [];
    const groups = [];
    for (let i = 0; ; ) {
      let replaced = false;
      path11 = path11.replace(/\{[^}]+\}/g, (m) => {
        const mark = `@\\${i}`;
        groups[i] = [mark, m];
        i++;
        replaced = true;
...
function buildMatcherFromPreprocessedRoutes(routes) {
  ...
  for (let i = 0, j = -1, len = routesWithStaticPathFlag.length; i < len; i++) {
    const [pathErrorCheckOnly, path11, handlers] = routesWithStaticPathFlag[i];
    if (pathErrorCheckOnly) {
      staticMap[path11] = [handlers.map(([h]) => [h, /* @__PURE__ */ Object.create(null)]), emptyParam];
    } else {
      j++;
    }
    let paramAssoc;
    try {
      paramAssoc = trie.insert(path11, j, pathErrorCheckOnly);
    } catch (e) {
...
var RegExpRouter = class {
  ...
  add(method, path11, handler) {
    ...
    const paramCount = (path11.match(/\/:/g) || []).length;
    if (/\*$/.test(path11)) {
      ...
      if (method === METHOD_NAME_ALL) {
        ...
        middleware[method][path11] ||= findMiddleware(middleware[method], path11) || findMiddleware(middleware[METHOD_NAME_ALL], path11) || [];
      }
      Object.keys(middleware).forEach((m) => {
  ...
  #buildMatcher(method) {
    ...
    [this.#middleware, this.#routes].forEach((r) => {
      const ownRoute = r[method] ? Object.keys(r[method]).map((path11) => [path11, r[method][path11]]) : [];
      if (ownRoute.length !== 0) {
        hasOwnRoute ||= true;
        routes.push(...ownRoute);
      } else if (method !== METHOD_NAME_ALL) {
        routes.push(
          ...Object.keys(r[METHOD_NAME_ALL]).map((path11) => [path11, r[METHOD_NAME_ALL][path11]])
        );
      }
    });
    if (!hasOwnRoute) {
...
var TrieRouter = class {
  ...
  add(method, path11, handler) {
    const results = checkOptionalParameter(path11);
    if (results) {
    ...
    this.#node.insert(method, path11, handler);
  }
  match(method, path11) {
...
var serveStatic = (options = { root: "" }) => {
  ...
  return async (c, next) => {
    ...
    let stats = getStats(path11);
    if (stats && stats.isDirectory()) {
      const indexFile = options.index ?? "index.html";
      path11 = join(path11, indexFile);
      stats = getStats(path11);
    }
    if (!stats) {
      await options.onNotFound?.(path11, c);
      return next();
    }
    const mimeType = getMimeType(path11);
    ...
    if (options.precompressed && (!mimeType || COMPRESSIBLE_CONTENT_TYPE_REGEX.test(mimeType))) {
      ...
      for (const encoding of ENCODINGS_ORDERED_KEYS) {
        ...
        const precompressedStats = getStats(path11 + ENCODINGS[encoding]);
        if (precompressedStats) {
          c.header("Content-Encoding", encoding);
          c.header("Vary", "Accept-Encoding", { append: true });
          stats = precompressedStats;
          path11 = path11 + ENCODINGS[encoding];
          break;
        }
      }
    }
    ...
    if (c.req.method == "HEAD" || c.req.method == "OPTIONS") {
      ...
      c.header("Content-Length", size.toString());
      result = c.body(createStreamBody(createReadStream(path11)), 200);
    } else {
      ...
      const stream2 = createReadStream(path11, { start, end });
      c.header("Content-Length", chunksize.toString());
      c.header("Content-Range", `bytes ${start}-${end}/${stats.size}`);
      result = c.body(createStreamBody(stream2), 206);
    }
    await options.onFound?.(path11, c);
    return result;
  };
};
...
var RepoConfigService = class {
   ...
   */
  generateRepoId(path11) {
    const hash = createHash("sha256").update(path11).digest("hex");
...
var LibraryClient = class {
   ...
   */
  async getAgentByPath(token, path11) {
    const response = await fetch(`${this.apiUrl}/api/agents/${path11}`, {
      method: "GET",
      headers: {
        "Accept": "application/json",
        "Authorization": `Bearer ${token}`
   ...
   */
  async storeAgent(token, path11, agent) {
    const response = await fetch(`${this.apiUrl}/api/agents/${path11}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
   ...
   */
  async archiveAgent(token, path11) {
    const response = await fetch(`${this.apiUrl}/api/agents/${path11}`, {
      method: "DELETE",
      headers: {
        "Authorization": `Bearer ${token}`
...
function createRepoRoutes(bridge) {
  ...
  app.get("/:repoId/agents", async (c) => {
    try {
      ...
      const agentsPath = join7(repo.path, ".claude", "agents");
      const agents = [];
      try {
        const files = await readdir5(agentsPath);
        const mdFiles = files.filter((f) => f.endsWith(".md"));
        for (const file of mdFiles) {
          const content = await readFile5(join7(agentsPath, file), "utf-8");
  ...
  app.get("/:repoId/skills", async (c) => {
    try {
      ...
      const skillsPath = join7(repo.path, ".claude", "skills");
      const skills = [];
      try {
        const entries = await readdir5(skillsPath, { withFileTypes: true });
        const directories = entries.filter((e) => e.isDirectory());
        for (const dir of directories) {
          const skillMdPath = join7(skillsPath, dir.name, "SKILL.md");
          try {
...
// packages/tiny-brain-dashboard/server/services/plugin-discovery.service.ts
...
var PluginDiscoveryService = class {
  ...
  constructor(knownRepos = []) {
    this.userPluginsDir = join8(homedir(), ".claude", "plugins");
  ...
  async discoverAll() {
    const [user, project, local] = await Promise.all([
      this.discoverUserPlugins(),
      this.discoverProjectPlugins(),
  ...
  /**
   * Discover plugins in user scope (~/.claude/plugins/)
   */
  async discoverUserPlugins() {
    return this.scanPluginDirectory(this.userPluginsDir, "user");
  }
  /**
  ...
  async discoverProjectPlugins() {
    ...
    for (const repoPath of this.knownRepos) {
      const pluginsDir = join8(repoPath, ".claude", "plugins");
      const plugins = await this.scanPluginDirectory(pluginsDir, "project", repoPath);
      if (plugins.length > 0) {
        result.set(repoPath, plugins);
      }
    }
    return result;
  }
  /**
   * Discover plugins in local scope (same path as project but different settings)
   * For now, we detect local scope by checking settings.local.json
   */
  ...
  async scanPluginDirectory(dir, scope, repoPath) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const pluginPath = join8(dir, entry.name);
  ...
  async parsePlugin(pluginPath, scope, repoPath) {
    const manifestPath = join8(pluginPath, ".claude-plugin", "plugin.json");
  ...
  async discoverComponents(pluginPath, manifest) {
    ...
    let hasHooks = false;
    let hasMcp = false;
    const skillsDir = manifest.skills ? join8(pluginPath, manifest.skills) : join8(pluginPath, "skills");
    if (existsSync2(skillsDir)) {
      try {
        const entries = await readdir6(skillsDir, { withFileTypes: true });
        for (const entry of entries) {
          if (entry.isDirectory()) {
            const skillMd = join8(skillsDir, entry.name, "SKILL.md");
    ...
    const agentsDir = manifest.agents ? join8(pluginPath, manifest.agents) : join8(pluginPath, "agents");
    ...
    const hooksPath = manifest.hooks ? join8(pluginPath, manifest.hooks) : join8(pluginPath, "hooks", "hooks.json");
    hasHooks = existsSync2(hooksPath);
    const mcpPath = manifest.mcpServers ? join8(pluginPath, manifest.mcpServers) : join8(pluginPath, ".mcp.json");
  ...
  async getPluginsForRepo(repoPath) {
    ...
    const localDir = join8(repoPath, ".claude", "local", "plugins");
    ...
    const projectDir = join8(repoPath, ".claude", "plugins");
...
`));
  ...
  const distPath = process.env.TINY_BRAIN_DASHBOARD_STATIC_PATH ? path8.resolve(process.env.TINY_BRAIN_DASHBOARD_STATIC_PATH) : path8.resolve(__dirname, "../dist");
  const distExists = fs7.existsSync(distPath);
  if (distExists) {
    app.use("/assets/*", serveStatic({
      root: distPath,
      rewriteRequestPath: (path11) => path11.replace("/assets", "/assets")
    }));
    ...
    app.get("/*", async (c) => {
      ...
      if (fs7.existsSync(indexPath)) {
        const indexHtml = fs7.readFileSync(indexPath, "utf-8");
...
var FileWatcherService = class extends EventEmitter2 {
  ...
  async start(watchPath, options = {}) {
    ...
    if (!fs8.existsSync(watchPath)) {
      throw new Error(`Watch path does not exist: ${watchPath}`);
    }
    const stats = fs8.statSync(watchPath);
    ...
    for (const filePath of previousFilesSet) {
      if (!currentFilesSet.has(filePath)) {
        ...
        const change = {
          type: "deleted",
          filePath,
          relativePath: path9.relative(this.watchPath, filePath)
        };
...
var FileWatcher = class {
  ...
  async start() {
    ...
    this.prdPath = path10.join(this.context.repositoryRoot, ".tiny-brain/progress");
    if (!fs9.existsSync(this.prdPath)) {
    ...
    this.fixesPath = path10.join(this.context.repositoryRoot, ".tiny-brain/fixes");
    if (fs9.existsSync(this.fixesPath)) {
  ...
  async handleFileChange(change) {
    try {
      ...
      const fileName = path10.basename(change.relativePath);
  ...
  async handleProgressChange(prdId, change) {
    try {
      ...
      try {
        ...
        const content = await fs9.promises.readFile(change.filePath, "utf-8");
